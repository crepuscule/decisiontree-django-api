...
def readFromDB(dbname="db_dataset",tablename='',fields=[]):
    db=pymysql.connect("47.95.196.25","root","12345678",dbname,charset="utf8")
    cur = db.cursor()     
    sql = "select "
    for value in fields:
        sql+="`%s`," % (value)
...
class ID3:
    ...
    def __init__(self,dataSource="db",sourceName="",fields=[]):
        ...
        if dataSource == 'csv':
            ...
            self.dataSet,self.labels = readFromDB("db_dataset",sourceName,self.fields)
        else:
    ...
    def CheckAccuracy(self,dataSource='',sourceName="",fields=[]):   
        ...
        if dataSource == 'csv':
            ...
            dataSet,labels = readFromDB("db_dataset",sourceName,fields)            
...
if __name__=='__main__':  #只有在执行当前模块时才会运行此函数
    ...
    ID3_controller = ID3("db","db_dataset.personal_transcripts_discrete_cs",fields=['AdvancedMath','LinearAlgebra','ProbabilityTheory','DataStructure','DataBase','ComputerNetwork','OperatingSystem','CompositionPrinciple','CppProgramming','ProgrammingPractice','JavaProgramming','CSorSE','NCRE_CPP2'])#四级,六级,政治面貌,计算机等级,综合能力,性别,去向
    ...
    print("--结果-->",ID3_controller.Classify([ 'excellent', 'excellent', 'excellent', 'excellent', 'excellent', 'failed', 'excellent', 'failed', 'excellent', 'excellent', 'good', 'failed'])) 
    ...
    accuracy = ID3_controller.CheckAccuracy('db','db_dataset.personal_transcripts_discrete_cs',['AdvancedMath','LinearAlgebra','ProbabilityTheory','DataStructure','DataBase','ComputerNetwork','OperatingSystem','CompositionPrinciple','CppProgramming','ProgrammingPractice','JavaProgramming','CSorSE','NCRE_CPP2'])
    print(accuracy)
